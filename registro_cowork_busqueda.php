...
if ($suscripcion == "A" || $suscripcion == "F") {
    ...
    if (empty($suscripcion) || is_null($suscripcion)) {
        // Acción específica cuando $suscripcion está vacío o es null
       $sql = "INSERT INTO registro (documento, entrada, servicio, estado, secuencial_planes ) VALUES ( '$documento', '$entrada', '$servicio', '$estado', '$sec_P' )";
       mysqli_query($conn, $sql);
        mysqli_close($conn); 
        header('Location: registro.php');
        // Aquí puedes redirigir o manejarlo según sea necesario
    } else {
